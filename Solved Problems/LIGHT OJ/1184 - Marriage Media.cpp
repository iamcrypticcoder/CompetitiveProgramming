...
bool isPossibleLink(PERSON p1, PERSON p2)
{
   if(abs(p1.height - p2.height) > 12) return false;
   if(abs(p1.age - p2.age) > 5) return false;
   if(p1.divorced != p2.divorced) return false;
   return true;
}
...
//    WRITE("output.txt");
   ...
   FOR(tc, 1, TC) {
      ...
      FOR(i, 1, M)
         FOR(j, 1, N)
            if(isPossibleLink(men[i-1], women[j-1])) {
               res[i][M+j] = 1;
               G[i].PB(M+j);
               G[M+j].PB(i);
            }
