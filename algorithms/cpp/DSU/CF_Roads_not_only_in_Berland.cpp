...
int main() {
    ...
    for (int i = 0; i < edges.size(); i++) {
        ...
        if (findSet(u) == findSet(v)) removableEdge.push_back({u, v});
        else unionSet(u, v);
    }

    set<int> rootSet;
    for (int i = 1; i <= N; i++) rootSet.insert(findSet(i));

    vector<int> roots;
    roots.assign(rootSet.begin(), rootSet.end());
    //for (int i = 0; i < roots.size(); i++) cout << roots[i] << endl;

    vector<pair<int, int>> newEdge = vector<pair<int, int>>();
    for (int i = 1; i < roots.size(); i++)
        newEdge.push_back({roots[i-1], roots[i]});

    printf("%d\n", newEdge.size());
    for (int i = 0; i < newEdge.size(); i++) {
        printf("%d %d %d %d\n",
               removableEdge[i].first, removableEdge[i].second,
               newEdge[i].first, newEdge[i].second);
    }
