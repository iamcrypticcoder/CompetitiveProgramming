/*
        Problem Link : https://www.spoj.com/submit/CLFLARR/
...
struct Query {
    int l, r, c;
    Query() {}
    Query(int x, int y, int z) : l(x), r(y), c(z) {}
};
...
int main() {
    ...
    scanf("%d %d", &N, &Q);
    queries = vector<Query>(Q, Query());
    for (int i = 0; i < Q; i++) {
        scanf("%d %d %d", &queries[i].l, &queries[i].r, &queries[i].c);
    }
    ...
    for (int i = Q-1; i >= 0; i--) {
        ...
        r = queries[i].r;
        c = queries[i].c;
        for (int u = findSet(l); u <= r; u = findSet(u)) {
            result[u] = c;
            dset[u] = u + 1;
        }
    }
