...
int maxDiff(vector<int> arr) {
    vector<int> rightNSE(arr.size(), 0);
    vector<int> leftNSE(arr.size(), 0);

    stack<int> st;
    st.push(0);
    for (int i = 1; i < arr.size(); i++) {
        while (!st.empty() && arr[i] < arr[st.top()]) {
            rightNSE[st.top()] = arr[i];
            st.pop();
        }
        st.push(i);
    }
    ...
    for (int i = arr.size()-2; i >= 0; i--) {
        while (!st.empty() && arr[i] < arr[st.top()]) {
            leftNSE[st.top()] = arr[i];
            st.pop();
        }
        st.push(i);
    }

    for (int i = 0; i < rightNSE.size(); i++) cout << rightNSE[i] << " ";
    cout << endl;
    for (int i = 0; i < leftNSE.size(); i++) cout << leftNSE[i] << " ";
    cout << endl;

    int ret = INT_MIN;
    for (int i = 0; i < arr.size(); i++)
        ret = max(ret, abs(rightNSE[i] - leftNSE[i]));
    return ret;
}
...
int main()
{
    ...
    arrList.push_back(vector<int> {2, 1, 8});
    arrList.push_back(vector<int> {2, 4, 8, 7, 7, 9, 3});
    arrList.push_back(vector<int> {5, 1, 9, 2, 5, 1, 7});

    for (int tc = 0; tc < arrList.size(); tc++) {
        cout << maxDiff(arrList[tc]) << endl;
    }
