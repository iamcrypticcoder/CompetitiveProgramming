...
void push(int val) {
    stackSize++;
    if (NULL == stackHead) {
        stackHead = new Node(val);
        minElement = val;
        return;
    }
    Node *newNode;
    if (val < minElement) {
        int x = 2 * val - minElement;
        minElement = val;
        newNode = new Node(x);
    } else {
...
int pop() {
    if (NULL == stackHead)
        return INT_MIN;
    Node *tNode = stackHead;
    int tVal = tNode->val;
    if (stackHead->val < minElement) {
        tVal = minElement;
        minElement = 2 * minElement - stackHead->val;
    }
