...
void sortStack(stack<int> &st) {
    ...
    while (!st.empty()) {
        ...
        int cnt = 0;
        while (!tmpStack.empty() && v > tmpStack.top()) {
...
int main()
{
    ...
    arrList.push_back(vector<int> {1});
    arrList.push_back(vector<int> {1, 2});
    arrList.push_back(vector<int> {1, 2, 3});
    arrList.push_back(vector<int> {1, 2, 3, 4});
    arrList.push_back(vector<int> {34, 3, 31, 98, 92, 23});

    for (int tc = 0; tc < arrList.size(); tc++) {
        stack<int> st;
        for (int i = 0; i < arrList[tc].size(); i++)
            st.push(arrList[tc][i]);
        sortStack(st);
