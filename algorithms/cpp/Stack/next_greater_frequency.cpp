...
vector<int> findNGF(vector<int> arr) {
    ...
    for (int i = 1; i < arr.size(); i++) {
        while (!st.empty() && freqArr[i] > freqArr[st.top()]) {
            ret[st.top()] = arr[i];
            st.pop();
        }
        st.push(i);
...
int main()
{
    ...
    arrList.push_back(vector<int> {1, 1, 2, 3, 4, 2, 1});

    for (int tc = 0; tc < arrList.size(); tc++) {
        vector<int> result = findNGF(arrList[tc]);
